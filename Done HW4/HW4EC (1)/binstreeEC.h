...
class BST
{
 ...
 public:
  ...
  // PURPOSE: these will show the vertices in POST order - same as Depth First
  // TO CALL: No parameter to ShowPostOrder but provide a pointer to
  //          the root vertex in calling PostorderTraversal
  void ShowPostOrder();
 ...
 protected: // utility functions follow - these are not for the clients to use
  ...
  // PURPOUSE: Called by the Insert finction to give the added leaf a height and balance factor
  void Number(Vertex*);
