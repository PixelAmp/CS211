...
//doesnt actually work, for some reason, when passed by refrence,
//it breaks my linked list when it gets returned, even if it doesn't do anything
//PARAMETER: list passed by value becomes copiedList
void CopyTest(slist copiedList)
{
  cout << "in copy test" << endl;

  cout << "list passed by reference" << endl;
  copiedList.displayAll();

  cout << "adding 6 to rear of copied list" << endl;
  copiedList.addRear(6);

  cout << "new list" << endl;
  copiedList.displayAll();
  int temp;
  copiedList.deleteRear(temp);
  cout << "finished" << endl;
}


//PURPOSE of the Program: To be able to implement and use an overloaded operator and a copy constructor to complete the linked list class.
//Algorithm/Design: Uses functions from slist and llist to do what the client requests
int main()
{
  slist L1, L2;
  int temp;
